...
| ブラウザで開く | 何が見える |
|---|---|
| 🗺️ **[feature-sitemap.html](feature-sitemap.html)** | **機能サイトマップ**。アプリ全体→分類(取得・記録・表示・読み上げ・診断…)→各機能の「何をして・どのファイルか」をカードで。AIも人間も全体像を掴むならまずここ。 |
| 🌳 **[code-tree.html](code-tree.html)** | **全ファイル網羅**。冒頭に「🦴 データの流れ（取得→記録→集計→表示）」、その下に全ファイルのツリー＋各ファイルが「何をするか」。畳めば俯瞰・開けば毛細血管。役割コメントが無いファイルは赤。 |
| 🎨 [repo-tree-map.html](repo-tree-map.html) | ディレクトリ役割＋機能逆引きの色付きカード。 |

> AI に渡すなら同じ内容のテキスト版: [feature-sitemap.md](feature-sitemap.md) ／ [code-tree.md](code-tree.md)。
