# 引き継ぎ 2026-08-17 — コメント47秒遅延（真因特定→修正済み）
...
**真因は Chrome の「裏タブ setTimeout クランプ(1/分)」。v0.1.1417 で修正済み。**

- 遅いのは受信側ではなく**送る側が起きられないこと**だった（受信側は実測5msで健全）
- 対処: 裏タブのときだけ「溜まったら吐く」しきい値を **240行 → 40行**
- 効果の見積り: 毎秒10行の配信で **24秒待ち → 4秒待ち**
- 可視中は**据え置き**（150msタイマーが正常に効いているので触る必要が無い）

★**未検証**: hidden 中の実測はこの環境では取れていない（下記「正直な限界」）。
　実配信で v0.1.1417 の `可視中の配達平均 / 裏タブN件` を読んで確認すること。
...
## 3. ~~★未解決の謎：受信+破棄 > 送信~~ → ★2026-08-19【決着・この節の結論は誤り】

> ★**この節の「二重注入が原因」は誤りだった。**
> 正本: [`HANDOFF-2026-08-19-instant-push-1.51x.md`](HANDOFF-2026-08-19-instant-push-1.51x.md)
>
> - ★下の「content-entry.js には二重注入を止める latch が無い」は**事実と異なる**。
>   latch は `content-entry.js:19302` に**存在する**（出荷 dist にも入っている）。
> - 真の説明: **送信は primary iframe 1つ宛に1回、受信は生存する iframe が各自1回ずつ**。
>   ＝ `sentCount` と `receivedCount` は**同じものを数えていない**ので比を取ること自体が誤り。
> - ★**CPU/メモリを1.5倍使っている、という読みも誤り**（postMessage は1回だけ）。
>   増えているのは**計器の数字だけ**で、実害は確認されていない。
> - 次の一手は「`duplicateSeen` を速報に出す」1行だけ（**既に数えている**が報告に乗っていない）。

以下は当時の記述（誤りを含む。経緯として残す）:
