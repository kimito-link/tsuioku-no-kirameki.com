...
<details><summary>🗂 このカテゴリの全担当ファイル(自動分類) 86</summary>
...
> changelog 全 368 版を「バグ系統」で束ねた枝。同系統をまた触るとき、過去の修正と「なぜ毎回触るか」を辿る(再発防止)。新しい順。
...
### 🏟 会場・席 (121版)
- `v0.1.1032` 2026-07-01 — 会場が変化のない時は描き直さず軽くした(重い配信対策)
- `v0.1.1031` 2026-07-01 — 会場のひな壇の上に「応援者トップ」バーを追加した
