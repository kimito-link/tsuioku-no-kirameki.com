...
const FEATURES = [
  ...
  { feature: '診断/ちくらん タブ+カードクリックで応援者展開', desc: '状態ページ上部に「📊診断/🏆ちくらん」タブ(body.tab-chikuran で診断系レーンを CSS 非表示・配信カードに集中)。各配信カードに details「🏆応援者ランキングを見る」=クリックで topSupporters を🥇🥈🥉展開。応援者データは popup で開いている配信ぶんだけ(reportPreview.liveId 一致)=その配信は展開・他は popup で開く案内(死にリンクにしない)。signature に reportPreview を含めて応援者到着時にカード再構築。将来の Kimito Link ランキングの入口(v0.1.869)', paths: ['src/extension/status-entry.js', 'extension/status.html', 'src/lib/supporterRanking.js'], tags: ['診断', '表示', 'ナビ'] },
