...
const FEATURES = [
  ...
  { feature: '応援ライブビュー(リアルタイム盛り上がり・新規タブ)', desc: 'ちくらんカードの「🔥応援ライブビューを開く」で live-view.html?lv=... を新規タブで開く(chrome.runtime.getURL)。chrome.storage を2秒購読し盛り上がり🔥(分速→computeHeatLevel)/応援者🏆/コメント数/来場をリアルタイム再描画・hot/blazing でバー脈打つ。データ取得を createLiveViewDataSource に隔離=将来サーバー公開版(拡張不要で URL 閲覧)へ移植可能(描画は不変)。Web/iOS/Android への土台(v0.1.871)', paths: ['extension/live-view.html', 'src/extension/live-view-entry.js', 'src/lib/heatLevel.js'], tags: ['表示', 'リアルタイム', 'レポート'] },
