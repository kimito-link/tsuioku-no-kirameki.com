...
const FEATURES = [
  ...
  { feature: '数字の自己矛盾の自動検知(self-verifying)', desc: '状態速報が自分の出した数字どうしを照合し、論理的に不可能/桁違いの食い違いを⚠に出す。コメントした人>来場・のべ別キー>本文数・レポート本文が記録総数の半分未満(過小集計の疑い)・記録が公式を大きく上回る(別配信混入/二重計上の疑い)。人が目で照合しなくても診断が自動で気づく(v0.1.859・statusActionAdvisor の対処カードに統合)', paths: ['src/lib/numberConsistency.js', 'src/lib/statusActionAdvisor.js'], tags: ['診断', 'レポート', '記録'] },
