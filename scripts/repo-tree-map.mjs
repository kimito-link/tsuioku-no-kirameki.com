...
const ROLES = {
  ...
  'docs/marketing': { role: '発信用の原稿(X 記事・告知文)。★数字は出典なしで書かない', tags: ['マーケ', '原稿'] },
...
const FEATURES = [
  ...
  { feature: 'ランキング(/live/)の配信ごと OGP', desc: 'vercel.json が「?lv= あり∧カード用クローラー UA」だけ /api/live-og へ rewrite。api は live:ranking:latest から該当配信を引き、og:image=配信サムネ(thumbnail.large)・og:title=liveOgTitle・og:description=liveOgDescription(来場・コメント・ギフト・広告の数字を liveOgStats が整形。0/欠落は省く)の最小 HTML を返す(リダイレクト無し・no-store)。人間は従来どおり静的 /live/。不在 lv は汎用カード。支援者名・コメント本文・個別ポイントは出さない(v0.1.1518)', paths: ['api/live-og.js', 'src/lib/liveOgHtml.js', 'src/lib/liveOgStats.js', 'src/lib/liveRankingView.js', 'vercel.json'], tags: ['LP', '公開', 'ランキング'] }
];
