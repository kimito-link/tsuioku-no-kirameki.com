...
const ROLES = {
  ...
  'tools': { role: '補助ツール(LP overflow 監査・MCP サーバ・OG カード画像合成 等)', tags: ['ツール'] },
...
const FEATURES = [
  ...
  { feature: 'ランキング(/live/)の配信詳細モーダル', desc: '配信カードの「🔍詳しく見る」ボタンで <dialog> モーダルを開き、拡張の会場モードに近い見た目(参加者アイコン・ギフト/広告/コメントの3枠)を既存データのまま大きく表示。新規APIは作らず /api/live-ranking の応答から findLive で該当配信を検索するだけ。60秒の全体更新(#list全置換)とは DOM 上分離し、load() の成否から safeDetailSync/safeDetailSyncError で結果を伝える。配信が一覧から消えても閉じずに最後の姿を凍結しバナーで通知(detailBanner)。URLは ?lv=<lv>&detail=1 に pushState/popstate 同期(council-fable設計・2026-09-26)', paths: ['src/lib/liveDetailView.js', 'src/extension/live-ranking-entry.js', 'tsuioku-no-kirameki/live/index.html'], tags: ['LP', '公開', 'ランキング'] }
];
