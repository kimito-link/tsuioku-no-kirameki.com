...
export function weightOf(label) {
  ...
  if (!n.startsWith("local/")) {
    ...
    //
    // ★2026-09-06 差し戻し(1→3): 上記の差し戻し条件を満たしたので実行する。
    //  実会議2回で lead として連続FAILED（いずれも HTTP 429 "Rate limit exceeded"）。
    //  単発プローブでも 20秒間隔4回すべて429、統括プロンプト4回もすべて429で、
    //  60秒待っても回復しなかった＝一時的なスパイクではない。
    //  ★プロバイダ全体ではなくモデル群の枠が枯れている: 同キーで codestral は 200、
    //   magistral-medium と magistral-small の2体だけが429。mistral障害ではない。
    //  一方 nemotron 側は同日に大きく回復していた（"hi" 5回で成功4/5・**中央値713ms**。
    //  09-02時点の 9〜61秒から改善）。統括プロンプトでも 1896/2950ms が出る。
    //  ただし503が混じり61秒に跳ねる回もあるため「速い」と断定はしない。
    //  ＝**どちらも単独では不安定**。weight で優劣を固定するより、
    //   nemotron(w2) → openrouter版(w3) → magistral-medium(w3) の順に
    //   自然に流れる状態へ戻すのが安全と判断した。
    //  再昇格の条件: magistral-medium が実会議2回連続で成功し、かつ単発プローブで
    //   429ゼロを別日に2回確認できたとき（無料枠の枠回復は日単位で変動するため）。
    //  ※この行を消さないのは、次に同じ判断をする人へ経緯を残すため（LINEUPの流儀に倣う）。
