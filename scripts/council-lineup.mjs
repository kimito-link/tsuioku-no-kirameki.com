...
export const LINEUP = [
  ...
  //
  // ★2026-09-23 実測更新（2026-08-25の保守計画の前提が古くなっていたので上書きする）:
  //  当時の計画は「現行メンバーの失敗は84件中1件(HTTP 503・再発なし)、timeoutはゼロ＝
  //  測定された問題が無い」と書いてメンバー入替を「やらないこと」に入れていた。**再発している。**
  //  全記録の実測: **17失敗/54発言 (31%)**、内容は一貫して `HTTP 503 Service temporarily overloaded`。
  //  ★CFトークン取り違え修正(09-16)より後だけに絞っても **4失敗/14発言 (29%)** で改善していない
  //   （同時期の cloudflare/gpt-oss-20b は 0失敗/12発言＝あちらの401は完全に解消済み。
  //     つまりこの503は「あの事故の残り」ではなく、このモデル固有の継続的な不安定さ）。
  //  timeoutが0件なのは当時のまま（＝タイムアウト延長の判断は正しかった。問題は別）。
  //  ★それでも今は撤去しない: 503は容量枯渇型で402/403の課金要求ではない（据え置き基準どおり）。
  //   かつ lead は nvidia + openrouter×2 の3経路が生存しており、落ちた回は openrouter 版が代打に入る。
  //  ★次に判断が変わる条件（これを満たしたら lead の主力入替を検討する）:
  //   (a) openrouter 版の :free スラッグがカタログから消える（＝代打が無くなる）
  //   (b) 09-17以降の失敗率が50%を超える（半分落ちるなら主力の意味がない）
  //   (c) 503 が 402/403 の課金要求に変わる
  //  ★★2026-09-26 再測定の結果: **改善している。据え置きで正しい。**
  //    全記録 27%(18/66) ／ **09-17以降 19%(5/26)** ／ **直近3日 8%(1/12)**。
  //    09-23時点の29%から下がっており、条件(b)には達していない（(a)(c)も未該当）。
  //    ★09-23に「31%は高い」と感じて撤去へ動いていたら、改善中のモデルを外していた。
  //     **日数や1回の測定で動かず、条件を数字で決めて測り直す**——これがこの節の価値。
  //     次も同じ3区間（全記録／09-17以降／直近3日）で測ること。
  //  ★数え方: `council/**/*.json` の `rounds` は **dict(`{routed:[...]}`)** であって配列ではない。
  //   配列として回すと key 文字列 'routed' を拾って**全件0件に見える**（2026-09-23に実際に踏んだ）。
  ...
  //  ★2026-09-26 再測(37日連続失敗・日報が撤去を推奨してきたので条件を実測で確認した):
  //   **8/25・8/27と完全に同一**。429×5・200×2で、200なのは今回も MiniMax-M2.7(812ms) と
  //   gemma-4-31B-it(980ms)。カタログも7件在籍のまま（DeepSeek-V3.1 も居る）。
  //   → (a)(b)(c) いずれも不成立。**撤去しない**。日報の推奨は日数だけを見た自動生成であり、
  //     ここの3条件が優先する（日報に従って外さないこと）。
  //   ★この日の注意: 最初に DeepSeek-V3.1 と Meta-Llama の2つだけ測って「全モデル429＝
  //    条件(a)成立」と早合点しかけた。**「全モデル」と書いたら本当に全部測る**
  //    （5体目で200が出て判定がひっくり返った）。
  //   ★生存2体は roleOf 上どちらも generalist（critic ではない）ので、
  //    死んでいる critic 予備の差し替えには使えない。かつ generalist は既に3体生存中で、
  //    足すと恒久ルール5（同役割の予備に同一プロバイダを重ね積みしない）に触れる。
