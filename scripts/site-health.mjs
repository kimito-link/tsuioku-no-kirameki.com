...
import { findBrokenInternalLinks } from '../src/lib/siteLinkHealth.js';
...
function render(files, broken) {
  const lines = [];
  lines.push('# サイト健全性チェック（自動生成）');
  lines.push('');
  lines.push('> `scripts/site-health.mjs` が git 追跡の公開ページ(LP/記事/docs/council)を静的解析。**手で編集しない**。');
  lines.push('> 検証: **相対内部リンク(.html/.md)の参照先がディスクに実在するか**。外部リンクは叩かない(依存/プライバシー/速度)。');
  lines.push('> 再生成: `npm run site-health` ／ 検証のみ: `npm run site-health:check`(リンク切れで exit 1)。');
  lines.push('');
  ...
  if (!broken.length) {
    ...
    lines.push('');
    lines.push('参照元 → リンク(解決先) の順。`scripts` でリネーム/削除したファイルへのリンクが残っている可能性。');
    lines.push('');
...
if (isCheck) {
  ...
  console.log(`[site-health] OK(対象 ${files.length} ファイル・リンク切れ 0)。`);
} else {
  mkdirSync(join(ROOT, 'docs'), { recursive: true });
  writeFileSync(OUT_MD, md, 'utf8');
  console.log(`[site-health] wrote ${OUT_MD}(対象 ${files.length}・リンク切れ ${broken.length})`);
  for (const b of broken) {
    console.warn(`[site-health] リンク切れ: ${b.from} → ${b.link}`);
  }
}
