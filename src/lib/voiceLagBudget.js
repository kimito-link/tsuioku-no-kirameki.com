 ...
 *
 * v0.1.1246(2026-08-04 実機実測): ノートPCで【1件あたり5847ms】が観測され、床2に張り付いて
 *   677件中266件(39%)が読まれずに捨てられていた。内訳は合成待ち1310ms / 再生準備48ms /
 *   実再生3922ms(=WAV申告3718msの1.05倍)で、【実再生が67%】。つまり詰まりではなく
 *   「朗読そのものに4秒かかっている」だけであり、PC性能の問題ではない。
 *
 *   このとき予算方式は原理的に機能しない: 予算は安全網(8000ms)未満でなければならず
 *   (下の不等式)、上限の7999msでも 7999/5847=1件 にしかならない。つまり
 *   【1件が数秒かかる配信では、予算がどう転んでも床の値がそのまま実効上限になる】。
 *   床2は「今+次」だけを意味し、リアルタイム性の代償として取りこぼしが常態化する。
 *
 *   わんコメ(OneComme)は limitQueue=10 固定・時間ゲート無しで実用に耐えている
 *   (voiceAgeGate.js 冒頭の実バイナリ解析)。追憶は安全網8000msを併せ持つため、
 *   床を上げても「何分も前を読み続ける」暴走にはならない(安全網が頭打ちにする)。
 *
 *   そこで床を引き上げる。天井8は据え置き(12への引き上げは却下済み案)。
 *
 * v0.1.1247(2026-08-04・同日の追加実測で【自己強化ループ】が確定したため 4→5):
 *   床4で出した速報が「合成待ち 1310→3343ms(2.5倍に悪化)・合成失敗52件が全部時間切れ・
 *   判定=coldsynth」を示した。これは下の VOICE_SYNTH_WAIT_DOMINANT_RATIO 節が
 *   【仮説B】として予告していたループそのもの:
 *
 *     実効上限2 → キューは最大2件 → resolveVoiceSynthDepth(2)=1(先読み無効)
 *       → 合成が再生と重ならず毎件コールドスタート → 合成待ちが伸びる
 *       → 処理時間が伸びる → 実効上限がさらに絞られる → 最初に戻る(ループが閉じる)
 *
 *   resolveVoiceSynthDepth は【キュー長5以上で深さ3】【3以上で深さ2】【それ未満は1】。
 *   つまり床4では深さ2止まりで、合成の重なりが完全には回復しない。
 *   床5にして初めて深さ3(=先読み最大)に到達し、ループが確実に切れる。
 *
 *   床5でも安全網8000msが先に効くため「何分も前を読み続ける」暴走にはならない。
 *   わんコメ(limitQueue=10固定)より依然として保守的な値である。
 */
export const VOICE_QUEUE_MAX_FLOOR = 5;
