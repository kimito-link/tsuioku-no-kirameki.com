...
export const RANK_STRIP_COMMENT_NOTE =
  'ユーザー別の応援件数が多い順です。下の「ギフト・投げ」とは別集計です。2位以降は当ストリップの1位（最多件）との差を表示します（本家ギフト履歴の番組累計ポイントや「ランキング」タブの貢献度とは別です）。';
...
/** ギフト帯の説明（本家はギフト履歴の番組累計／ランキングタブの貢献度と別。当帯は検知回数） */
export const RANK_STRIP_GIFT_NOTE =
  'ユーザー別の投げ／ギフト検知回数の多い順です（単位は回。ニコ生「ギフト」→「履歴」の番組累計ポイント・pt付の一覧とは別指標）。上の「応援コメント」とは別集計です。「ランキング」タブの貢献度（貢）とも別です。2位以降は当ストリップ内の最多回との差を表示します（番組累計のptではありません）。';
