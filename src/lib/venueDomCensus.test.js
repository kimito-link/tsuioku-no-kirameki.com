...
describe('collectVenueLaneDomCensus', () => {
  it('席収容タイル+裸タイルを可視として数え、keys を文書順で返す', () => {
    ...
    const c = collectVenueLaneDomCensus({ laneEls, stackEl: stack });
...
describe('countVenueKeyDuplicates', () => {
  it('同一段内の二重占有=dupIntra / 段×段=dupCross', () => {
    ...
    expect(dup).toEqual({ dupIntra: 1, dupCross: 1 });
  });
  ...
  it('重複が無ければ全部0', () => {
    ...
    expect(dup).toEqual({ dupIntra: 0, dupCross: 0 });
  });
});
