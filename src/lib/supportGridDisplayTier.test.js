...
import {
  ...
  demoteNiconicoAnonymousFromKontaTier,
...
describe('demoteNiconicoAnonymousFromKontaTier', () => {
  it('a: のみ konta を tanu に落とす', () => {
    expect(
      demoteNiconicoAnonymousFromKontaTier('a:abcdefghij', SUPPORT_GRID_TIER_KONTA)
    ).toBe(SUPPORT_GRID_TIER_TANU);
    expect(
      demoteNiconicoAnonymousFromKontaTier('715502', SUPPORT_GRID_TIER_KONTA)
    ).toBe(SUPPORT_GRID_TIER_KONTA);
    expect(
      demoteNiconicoAnonymousFromKontaTier('a:abcdefghij', SUPPORT_GRID_TIER_LINK)
    ).toBe(SUPPORT_GRID_TIER_LINK);
  });
});
...
describe('explainSupportGridDisplayTier', () => {
  it('supportGridDisplayTier と一致しフラグを返す', () => {
    ...
    expect(ex.tier).toBe(SUPPORT_GRID_TIER_LINK);
    expect(ex.strongNick).toBe(true);
    expect(ex.hasPersonalThumb).toBe(true);
    expect(ex.hasAnyAvatar).toBe(true);
    expect(ex.demotedAnonymousLinkToKonta).toBe(false);
  });

  it('匿名 ID でも強ニック＋個人サムネなら link（demoted は互換で常に false）', () => {
    ...
    expect(ex.hasAnyAvatar).toBe(true);
    expect(ex.demotedAnonymousLinkToKonta).toBe(false);
    expect(ex.demotedAnonymousKontaToTanu).toBe(false);
  });

  it('ルール上 konta の a: は demotedAnonymousKontaToTanu が true', () => {
    const ex = explainSupportGridDisplayTier({
      userId: 'a:abcdefghij',
      nickname: 'のらねこ',
      httpAvatarCandidate: ''
    ...
    expect(ex.demotedAnonymousKontaToTanu).toBe(true);
  });
});
...
describe('supportGridDisplayTier', () => {
  ...
  it('サムネのみ（匿名ニック・a:）は tanu（ニコ匿名 ID はこん太に載せない）', () => {
    expect(
      supportGridDisplayTier({
        userId: 'a:abcdefghijklmnop',
        nickname: '匿名',
  ...
  it('匿名の自動名＋良サムネも a: は tanu（こん太に載せない）', () => {
    expect(
      supportGridDisplayTier({
        userId: 'a:AbCdEfGhIjKlMnOp',
  ...
  it('数値 ID＋自動名＋良サムネも konta（link にしない）', () => {
    expect(
      supportGridDisplayTier({
        userId: '88210441',
        nickname: 'user 0539Z74OJ13',
  ...
  it('数値 ID + 強いニック + 表示用のみ canonical（stored 空）なら link（数値ID＋表示名で十分）', () => {
    const uid = '124172391';
  ...
  it('数値 ID + 強いニック + DOM 観測アバター（stored あり）なら link', () => {
    const uid = '124172391';
    const syn = niconicoDefaultUserIconUrl(uid);
    expect(
      supportGridDisplayTier({
  ...
  it('数値 ID + 強いニック + アバターなしでも link（数値ID＋表示名で十分）', () => {
    expect(
      supportGridDisplayTier({
        userId: '124172391',
        nickname: 'ぱん',
  ...
  it('a: ID + 強ニックのみは tanu（こん太は数値アカウント中心・匿名はたぬ姉へ）', () => {
    expect(
      supportGridDisplayTier({
        userId: 'a:abcdefghij',
        nickname: 'のらねこ',
...
describe('avatarObserved ベースの列判定（コンポーネント化）', () => {
  it('avatarObserved=true なら link（URL 分析に依存しない）', () => {
    expect(
      supportGridDisplayTier({
        userId: '25221924',
        nickname: 'レコ',
  ...
  it('avatarObserved=true + canonical URL でも link（URL 形式は無関係）', () => {
    const uid = '25221924';
    const syn = niconicoDefaultUserIconUrl(uid);
    expect(
      supportGridDisplayTier({
  ...
  it('avatarObserved=true + 匿名ID でも link（DOM でアバターが見えたなら）', () => {
    expect(
      supportGridDisplayTier({
        userId: 'a:AbCdEfGhIjKlMnOp',
        nickname: 'のら',
  ...
  it('avatarObserved なし + 強いニックネーム + 数値ID → link', () => {
    expect(
      supportGridDisplayTier({
        userId: '88210441',
        nickname: 'ぱん',
  ...
  it('avatarObserved なし + 匿名ID + 強いニック → tanu（こん太に載せない）', () => {
    expect(
      supportGridDisplayTier({
        userId: 'a:abcdefghij',
        nickname: 'のらねこ',
...
describe('explainSupportGridDisplayTier スナップショット（全フィールド検証）', () => {
  it('avatarObserved=true: link の全フラグ', () => {
    expect(
      explainSupportGridDisplayTier({
        userId: '25221924',
        nickname: 'レコ',
      ...
      hasAnyAvatar: true,
      avatarObserved: true,
      demotedAnonymousLinkToKonta: false,
      demotedAnonymousKontaToTanu: false,
      httpCandidateNonEmpty: false,
      storedAvatarNonEmpty: false
    });
  });

  it('avatarObserved=false + 強ニック + 数値ID: link の全フラグ', () => {
    expect(
      explainSupportGridDisplayTier({
        userId: '88210441',
        nickname: 'ぱん',
...
describe('canonical URL フォールバックによる tier 昇格', () => {
  it('数値ID + 強ニック + canonical stored → link（hasObservedAvatar 経由）', () => {
    const uid = '141919418';
