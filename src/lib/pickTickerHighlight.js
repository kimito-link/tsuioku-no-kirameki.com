...
/**
 * これ未満の本文はスパム扱いで候補外(「w」「88」等)。ギフトは免除。
 *
 * ★v0.1.1337: 4 → 3 に緩めた(実機で拾えなさすぎた)。
 *   実測(2026-08-11・状態速報): filteredTooShort=36 に対し scored=3。
 *   弾かれた数が採用の12倍で、枠がほぼ常に空(案内文のまま)だった。
 *   ニコ生の実際のコメントは「うゆゆ」「まじ卍」など3文字が多く、4字下限は厳しすぎる。
 *   ★2文字以下は残す=「w」「草」「88」は引き続き弾く(ここを外すと枠が草で埋まる)。
 */
export const TICKER_MIN_TEXT_LEN = 3;
