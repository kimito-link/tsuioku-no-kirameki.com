...
describe('★実物のLPで判定する(通し検査)', () => {
  ...
  it('★なふだの説明が「放送者さんだけ」と言っている(誤った説明への退化を止める)', () => {
    ...
    // ★ユーザー指摘「動く場合、自分のサムネがコメント欄にでる」＝見える結果を先に言う
    expect(near).toContain('コメント欄にあなたのアイコンと名前が出ます');
    // ★事実の但し書き。なふだは公式の設定で、この拡張の記録には一切載らない
    //   (nameplateToggleBoot.js の設計コメント / commentRecord.js に nameplate は0件)。
    //   ★ユーザー案「追憶のきらめきで自分の情報がのるよ」は事実でないので採用しない。
    expect(near).toContain('この拡張の記録には出ません');
  });
});
