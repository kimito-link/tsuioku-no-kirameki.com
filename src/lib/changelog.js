...
export const EXTENSION_CHANGELOG = Object.freeze([
  Object.freeze({
    version: '0.1.521',
    date: '2026-06-01',
    summary: '記録が止まったら自動で気づいて自動で復旧する仕組みを追加',
    items: Object.freeze([
      '複数の放送を同時に開いているときなどに「公式のコメント数は増えているのに、記録だけ途中で止まる」状態を自動で検知するようにしました',
      '止まりを検知すると、手動の操作なしで段階的に自動復旧します（保存の即時実行 → 保存方式の作り直し〔書き込みの軽量化リトライ〕→ ページに依存しない独立経路での取得開始）。記録がまた増え始めれば自動で通常状態に戻ります',
      'これまで必要だった「コンソールを開いて確認」「タブを更新（F5）」といった手動確認を不要にすることが狙いです',
