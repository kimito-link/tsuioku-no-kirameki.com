...
export const EXTENSION_CHANGELOG = Object.freeze([
  Object.freeze({
    version: '0.1.359',
    date: '2026-05-25',
    summary: 'イベント未参加なのに順位/スコアが出る誤表示を解消',
    items: Object.freeze([
      '[修正] イベントに参加していない普通の配信なのに「○○さんが参加しています！」「イベント現在順位 現在N位」「イベント累計スコア」などが誤って表示されることがある問題を直しました。配信ページに実際のイベント参加表示（公式の「参加しています」バナー等）が出ているときだけイベント情報を表示し、それが無いときは出さないようにしました（配信中で実際に参加している場合の表示はこれまでどおりです）',
      '[修正] イベント名がまれに文字化けして表示されることがある問題を直しました。内部データの一部を誤って名前として読んでしまうことがあったため、文字として正しく読めないものは名前に使わないようにしました',
      '[修正] イベント未参加の配信で、空の「イベント累計スコア」「イベント現在順位」の枠だけが残って縦の余白を取っていた問題を直しました。表示できる中身が無いときは枠ごと畳むようにしました'
    ])
  }),
  Object.freeze({
    version: '0.1.358',
    date: '2026-05-25',
    summary: '複数タブで同じ配信を開いた時の取り違えを修正',
    items: Object.freeze([
      '[修正] 同じ配信を複数のタブで開いているとき、コメントの送信や情報の取得が「いま見ているタブ」ではなく別の待機中のタブに向いてしまうことがある問題を直しました。タブのURLに付く小さな差（共有経由などで付く ?from=... のようなパラメータ）で別タブ扱いになり、最近見ていたタブが後回しになるのが原因でした。同じ配信のタブはまとめて扱い、直前に見ていた／前面にあるタブを優先するようにしています（1つのタブだけのときは影響ありません）'
    ])
  }),
  Object.freeze({
    version: '0.1.357',
    date: '2026-05-25',
    summary: '別ウィンドウ表示の下半分の大きな空白を解消',
    items: Object.freeze([
      '[修正] 別ウィンドウ表示で「配信を見ていない＋過去の応援履歴あり」のとき、上半分に前回の記録カードが出るのに下半分が大きな白い空白になっていた問題を直しました。配信中だけ意味がある公式値レーン（貢献度ランキングなど）が、配信を見ていないときも中身ゼロのまま大きな枠を確保していたのが原因で、配信を見ていないときはこれらの枠をたたんで前回サマリだけをすっきり表示するようにしました（配信中の表示はこれまでどおりです）'
    ])
  }),
  Object.freeze({
    version: '0.1.356',
    date: '2026-05-24',
    summary: '拡張更新時のエラーとコメント記録まわりの不具合を修正',
    items: Object.freeze([
      '[修正] 拡張を更新した直後に、前から開いていたニコ生のタブで「Cannot read properties of undefined (reading onMessage)」というエラーがまれに出ることがある問題を直しました。更新の瞬間に古いタブが一時的に拡張とつながれなくなるためで実害はありませんが、エラーが出ないようにしました（更新後に出ていたタブは1回再読み込みすると消えます）',
      '[修正] 配信を切り替えた直後に、新しい配信の最初のサマリ記録がまれに保存されないことがある問題を直しました（記録の間隔制限を配信ごとに分けました）',
      '[修正] コメント記録をクリアした後、内部の配信IDが古いまま残り、同じ配信で記録を再開したときに切り替え扱いにならない不整合を直しました',
      '[改善] コメントの盛り上がり（同じコメントの連続）を検出する精度を上げました。半角カナ・全角英数・カタカナ/ひらがなの表記ゆれ（例「ｵﾂ」「オツ」「おつ」）を同じものとして数えるようにしています（分析・レポート向け）',
      '[改善] 内部キャッシュが上限に達したときの掃除の順番を見直し、期限切れの古いデータから先に捨てるようにしました（記録される内容や表示はこれまでとまったく同じです）'
    ])
  }),
  Object.freeze({
    version: '0.1.355',
