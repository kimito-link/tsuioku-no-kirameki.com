...
export const EXTENSION_CHANGELOG = Object.freeze([
  Object.freeze({
    version: '0.1.250',
    date: '2026-05-15',
    summary: '貢献度ランキング検出と診断の改善',
    items: Object.freeze([
      'ギフトサイドバー自動オープン後の待ち判定を、画面の貢献度リストの実スクレイプと同じ条件に揃えました（新しい公式DOMだけが出ている配信でも検出漏れしにくくなります）',
      '自動オープンが失敗したとき、診断に「ランキングタブが見つからなかった」か「タブは押したが所定時間内にリストが出なかった」かを区別して出るようにしました',
      '開発者向け: `hasContributionRankingDomSignal` を追加し、単体テストで新旧DOMの両方を検証しています'
    ])
  }),
  Object.freeze({
    version: '0.1.249',
    date: '2026-05-15',
    summary: 'MCP 向け snapshot の診断と順位要約',
    items: Object.freeze([
      'ローカル保存する MCP 用データに、貢献度ランキングと広告ランキングの要約（順位・ポイント・匿名かどうかのみ。名前は含みません）が入ることがあります',
      '配信 ID とページの食い違い、または公式イベント周りの画面情報が一定時間より古い場合など、理由を短文コードで残します',
      '手元の MCP サーバーに「順位要約だけ取り出す」ツールを追加しました（拡張本体のバージョンと番号を揃えています）'
    ])
  }),
  Object.freeze({
    version: '0.1.248',
