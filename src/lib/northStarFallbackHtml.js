 ...
 * outerHTML が取れない時に、NDGR stats 由来の数値（`officialNicoEventRankNdgr` /
 * `officialEventGiftScoreNdgr` 等）から niconico の表示形式を模した最小 HTML を
 * 組み立てる純関数。
 *
 * 設計判断（v0.1.241 で確立、2026-05-11）:
 * - 「公式値の鏡」原則は維持: NDGR は niconico サーバが計算した正本、解釈ミスゼロ
 * - 「鏡のように貼り付け」原則は妥協（outerHTML を映すのではなく、数値から組み立て）
 *   → DOM が取れない時の fail-soft fallback として正当化、優先度は鏡 > NDGR > placeholder
...
/**
 * NDGR rank (1, 2, 3, ...) を niconico 風の「現在 N 位」HTML に整形する。
 *
