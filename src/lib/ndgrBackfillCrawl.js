 ...
 *
 * v0.1.749(会議+実機根治): 12 → 40 に引き上げ。実機(2026-06-15・あやりん配信 17%=124/727 で
 *   「過去ログは今は遡れませんでした(backward_exhausted・残り約598件)」)で、若い配信の序盤に
 *   コメントの無い空区画が 12 連続を超えると、その先に本物のコメントがあっても届く前に諦めて
 *   598件を残して止まっていた(複数配信並列で巨大裏配信にスロットを食われ序盤の余裕が無いほど顕著)。
 *   40 なら序盤の典型的な空区画を跨いで本物のコメントに届く。240(=分単位)までは上げず、本当に
 *   コメントの無い配信は ~6秒(40×150ms)で諦めて外側の fresh re-seed retry に任せる(体感維持)。
 *   ⚠️ 撤廃して240統一はしない(会議でも「本当に存在しない配信で無限リトライ」の懸念)。回数で
 *      バウンドしつつ序盤ギャップを跨ぐ最小の一手。
 */
export const NDGR_BACKFILL_COLD_RETRY_MAX = 40;
