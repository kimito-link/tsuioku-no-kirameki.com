 ...
 *
 * ★v0.1.1411: 4,000 → 8,000。v0.1.1410 で 4,000 に締めたが**締めすぎだった**。
 *   `_slice()` は引数なしだと残り全部を返すため、
 *   最初の read が予算を食うと後続が 0(読まない)になる。
 *   実測の正常系(合計 ≒ 4.2秒)を下回る予算にすると、
 *   **普通に動いている状態で読み取りが枯渇する**。
 *   ＝ 予算は「事故の上限」であって「体感の調整つまみ」ではない。
 *   体感は別の手段(コア先行描画)で解く。
 */
export const REFRESH_CYCLE_BUDGET_MS = 8_000;

/**
 * 初回サイクルの予算[ms]。**ページが開くまでの時間はこれで決まる**。
 *
 * ★v0.1.1411 で 1,500 → 5,000 に戻した(1,500 は退化だった)。
 *
 * ■ なぜ 1,500 だと壊れたか（実機で「読み込み中です...」が居座った）
 *   `_slice()` は **引数なしだと「残り全部」を返す**(next(0) → left)。
 *   status-entry のコア5read は全て `_slice()` を引数なしで呼ぶので、
 *   **最初の1本が予算を丸ごと持っていく**。
 *   初回は storage が最も混んでいるため lives/summaries で 1,500ms を使い切り、
 *   後続(fastDiag/popupDiag/backfill)が全部 `0` = 読まない になった。
 *   ＝ 初回に限りキャッシュ(stale)も無いので、**空のまま描画** →
 *     「読み込み中」が消えない。
 *   ★[[gate-may-be-the-only-recovery-path-2026-08-04]]:
 *     締切を厳しくする前に「それが唯一の供給経路でないか」を確かめること。
 *
 * ■ 5,000 の根拠
 *   実測の初回は popupDiag 1471 + extras 1454 + summaries 1278 ≒ 4.2秒。
 *   ここを下回ると上記の枯渇が起きる。**事故(80秒/12秒)は防ぎつつ、
 *   正常系は全部通す**値にする。体感の短縮は締切ではなく
 *   「コアを先に描いて追加データを後回しにする」で解く(未実装・§次の一手)。
 */
export const REFRESH_FIRST_CYCLE_BUDGET_MS = 5_000;
