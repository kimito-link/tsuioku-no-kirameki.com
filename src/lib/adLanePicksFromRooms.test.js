...
describe('★広告メッセージのラベル(2026-08-18)', () => {
  it('★メッセージだと読めた行だけ ID 行が「広告メッセージ」になる', () => {
    const out = adLanePicksFromRooms(
      [{ userKey: '__anon_ad_0', nickname: 'コメリにも１６ｃｍ自慢行くの？', rankHint: 3 }],
      io
    );
    expect(out[0].meta.idLine).toBe('広告メッセージ');
    // ★本文は一切変えない(原文がそのまま画面に出る)
    expect(out[0].meta.nameLine).toBe('コメリにも１６ｃｍ自慢行くの？');
    expect(out[0].title).toBe('コメリにも１６ｃｍ自慢行くの？');
  });

  it('★変わらない方も断言する(両方向を見ないと恒真になる)', () => {
    const out = adLanePicksFromRooms(
      [
        { userKey: '144514252', nickname: 'ゲスト', rankHint: 1 },
        { userKey: '__anon_ad_1', nickname: 'とねりん', rankHint: 2 },
        { userKey: '__anon_ad_2', nickname: 'いっくん応援団', rankHint: 4 }
      ],
      io
    );
    expect(out[0].meta.idLine).toBe('広告');       // 記名=名前
    expect(out[1].meta.idLine).toBe('#2');         // 短い名前=判定不能
    expect(out[2].meta.idLine).toBe('#4');         // 人間でも割れる=判定不能
  });

  it('★絵文字のみ・英数字のみでも壊れない', () => {
    const out = adLanePicksFromRooms(
      [
        { userKey: '__anon_ad_0', nickname: '💖✨', rankHint: 1 },
        { userKey: '__anon_ad_1', nickname: 'GET NOW 2026!', rankHint: 2 }
      ],
      io
    );
    expect(out[0].meta.idLine).toBe('#1');             // 判定不能=従来表示
    expect(out[1].meta.idLine).toBe('広告メッセージ');   // 終端記号つきの長い文
  });
});
