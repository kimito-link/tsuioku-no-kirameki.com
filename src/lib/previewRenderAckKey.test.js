...
describe('previewRenderAckKey', () => {
  ...
  it('ready/ts/liveId を正規化(件数は既定0)', () => {
    expect(buildPreviewRenderAck({ ready: true, liveId: 'LV123', nowMs: 5000 })).toEqual({
      ready: true, ts: 5000, liveId: 'lv123', laneTiles: 0, supporterRows: 0, gen: 0
    });
  });
  it('未指定は安全な既定(ready:false/ts:0/liveId:空/件数0)', () => {
    expect(buildPreviewRenderAck()).toEqual({ ready: false, ts: 0, liveId: '', laneTiles: 0, supporterRows: 0, gen: 0 });
    expect(buildPreviewRenderAck({ ready: 'yes' })).toEqual({ ready: false, ts: 0, liveId: '', laneTiles: 0, supporterRows: 0, gen: 0 });
  });
  ...
  it('v0.1.1056: gen(①が書いた鏡世代)を正規化', () => {
    expect(buildPreviewRenderAck({ ready: true, gen: 7 })).toMatchObject({ gen: 7 });
    // 負値・非数は 0 に丸める。
    expect(buildPreviewRenderAck({ gen: -3 })).toMatchObject({ gen: 0 });
    expect(buildPreviewRenderAck({ gen: 'x' })).toMatchObject({ gen: 0 });
  });
});
