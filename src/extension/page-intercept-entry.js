...
(() => {
  ...
  /*
   * ★v0.1.1276: 点滅の犯人を探すための同期トラップ(v0.1.1268〜1272)を撤去した。
   *
   *   撤去理由:
   *     ・点滅は Side Panel への移行(v0.1.1275)で解決した=犯人特定はもう不要
   *     ・このトラップは MAIN world に【重い監視を2つ常駐】させていた:
   *         - documentElement を subtree:true で MutationObserver
   *           (ニコ生はコメントが滝のように流れるので毎秒何百回も発火する)
   *         - 2秒ごとの setInterval(永久に止まらない)
   *     ・ユーザー報告「コメント送信を押してもなかなか送信されない」の原因は
   *       これらがページを重くしていたことと考えられる。
   *       コメント送信は同じ MAIN world(この直上の NLS_INTERCEPT_COMMENT_POST 経路)を通る。
   *
   *   ★教訓: 調査用の計器を出荷したまま放置しない。用が済んだら必ず外す
   *     ([[instrument-spiral-25-versions-2026-08-06]])。
   */
})();
