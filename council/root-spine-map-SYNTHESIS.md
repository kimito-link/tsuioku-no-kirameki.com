...
### ⚠️ 実装後の方針変更(2026-06-20・ユーザー指示)= 1枚に統合

当初は独立 `docs/spine-map.html` を作ったが、ユーザーが添付画像のツリー構造を示し「**毛細血管まで完全網羅**で各
ファイルが何をするか分かるツリーが1枚あればエラーはほぼ無くなる」と要望 → `docs/code-tree.html`(全ファイル
ツリー+役割自動抽出)を作成。その後「**1つにまとめてほしい**」との指示で、**spine-map.html/md は削除し、背骨
(取得→記録→集計→表示の流れ)を code-tree.html の冒頭セクションに統合**して1枚化した。storage キーの断線検知
(broadcaster バグ型)は独立図でなく既存の機械ゲート `feature-map:check` が担う(リサーチ①「ミスを止めるのは
ゲート」と整合)。**最終成果物 = `docs/code-tree.html`(+ code-tree.md)1枚**。
