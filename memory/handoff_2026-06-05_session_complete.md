...
## 🔬 退行の実機証拠(2026-06-05 status スクショ・v0.1.632 build 0605-010835)
3配信同時視聴の status で退行が数値に出ている:
- `lv350679746`(公式947)→ **記録1 / 取得率0%・最終取込2秒前**=録ってるのに backfill が進まず一気に取れてない(退行そのもの)。取得●●●●●(健康スコアは満点表示だが実態は0%=スコア基準が甘い可能性も別途要確認)。
- `lv350681227`(今川ギララ・公式3,300)→ 記録2,664 / **取得率81%**・paint 35ms/描画353回(アクティブ)=途中で頭打ち。
- `lv350681441`(なかにし・公式295)→ 記録302 / **取得率102%**・paint 7ms=軽い配信は完走。
→ **重い/複数配信ほど取得率が100%に届かず止まる**。visibility_paused 初回30秒沈黙の症状と一致。
別スクショ: パネル本体(今川ギララ watch)で「記録 2,617/3,254(80.4%)取得中」=やはり80%付近で停滞。

## 💡 ユーザー提案: 検証に「取得スピード」と「記録の%」を入れる(次セッションで実装)
退行(一気に取れない)を**自動検出**できるよう、status の見える化 or e2e/計測に以下を追加する案:
- **取得スピード**: 単位時間あたりの記録増加件数(records/sec)。backfill が走ってれば高い・沈黙してれば0付近。perfDiag に `recordRate` 的フィールド追加 → status の⚙行や健康チェックに。
- **記録の%(取得率)**: 既に status に出ている。⚠️**要確認の別バグ疑い**: lv350679746「取得率0%」表示なのに健康チェック「取得 ●●●●●」満点。`scoreCaptureRate` 自体は確認済みで正しい(0%→return 0=●0個・liveHealthScore.js:28-37)。よって**status 側で officialRatePct が 0 として buildLiveHealth に渡っていない**疑い(記録1/公式947 だが rate が null or 別値)。status-entry.js の summarizeOneLive の officialRatePct 計算→カードへ渡す経路を要確認。
- これがあれば「退行で取得が止まった配信」が status で赤く見え、手で気づける+将来 e2e で「開いて N 秒後に取得率が上がること」を自動検証できる。

## 次セッション入口(優先順)
1. ✅ **bump v0.1.632 commit/push 済**(9e51c41)
2. 🔴 **退行修正**(visibility_paused クールダウンの初回30秒沈黙・content-entry.js:15429-15447)← 最優先・実機再現(配信開いて30秒沈黙)してから
3. **検証に取得スピード+記録%を追加**(ユーザー提案・退行の自動検出。perfDiag に recordRate、健康チェックの取得スコアを取得率に厳密連動)
4. 大量コメント白化(renderUserRooms 冪等化)
5. Web版 Vercel 設定(Upstash/env/app ドメイン)でスマホ閲覧を通す
