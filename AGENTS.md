...
### 3.6 外部 API は「いつか落ちる・いつか形が変わる」前提で書く（2026-08-10 ユーザー確立）

**サードパーティのレスポンスが絶対に成功する前提で書くな。** この拡張はニコニコの
複数 API（NDGR / nicoad / koken / usericon CDN）と公式 DOM に依存しており、
**外部サービスは予告なく落ちるし仕様も変わる**。道連れで落ちない作りにすること。

- **必ず守る**
  - レスポンスは**形から検証**してから使う（`isLikelyNicoadRankingShape` が手本＝
    `meta.status` と配列キーを見る。koken は `rankers` / nicoad は `ranking` で、
    取り違えると全件 empty になる実績あり）。
  - **フィールドが消えても壊れない**。存在しない＝`undefined` を「値が false」と
    混同しない。`hasNoIcon` のような真偽フラグは「無い」と「false」を区別して扱う。
  - **fail-soft**: 正規化に失敗したら `null` を返し、呼び出し側は
    「rows>0 のときだけ既存 storage を上書き」する（空配列で既存値を clobber しない）。
  - **代替表示を必ず用意する**。人物なら identicon（ゆっくり顔）、数値なら
    「取得中」「—」。空白やエラー文字列をそのまま画面に出さない。
  - **画像 URL も外部依存**。CDN は 404 を返す（アイコン未設定・退会）。
    `<img>` には必ず load 失敗時のフォールバックを付ける。
  - 全体を `try/catch` で握りつぶすのではなく、**壊れた1件だけ捨てて残りを描く**
    （1行の異常で応援レーン全体を落とさない）。

- **実例（この原則が生まれた経緯・2026-08-10 v0.1.1307）**
  公式 nicoad API はアイコン未設定の広告主にも `thumbnailUrl` を返すが、中身は
  `usericon/defaults/blank.jpg`（のっぺらぼう）だった。正規化がこの情報を捨てたため
  下流が uid から CDN URL を導出し、**実体が無く 404 →壊れ画像が白丸として並んだ**
  （実データ10件中7件が該当）。「サムネURLが来ている＝画像がある」という
  暗黙の前提が誤りだった、という典型例。

- **禁止**
  - 外部 API の応答を**そのまま UI に流し込む**（検疫なしの `innerHTML`・
    未検証 URL の `src` 代入）。
  - 「今は必ず入っているフィールド」に依存した必須アクセス（`row.data.ranking[0].userId`）。
  - 障害時に**画面が空白のまま無言**になる作り。必ず「取得中」か理由を出す。
...
**⚠️ リリース工程の禁止事項(2026-07-06 実事故から明文化):**
- **配信視聴中の `copy:ext` 禁止**。未パック拡張は「本体=リロード時のまま・ページ/iframe=ディスクを
  都度読む」ため、ディスクだけ差し替えると**新旧混在ランタイム**になり、コメント送信不可等が黙って
  発生する(実測: 本体1077+ページ1080)。パッチは溜めてから copy:ext は1回、直後にユーザーの
  拡張リロードとセットで行う(v0.1.1082 で混在検知バナー+copy:ext 警告を実装済み。バナーが出たら
  即リロード)。
- **`git add` は新規ファイルを明示列挙**。`git status | grep -v '^??'` のようなフィルタで add すると
  新規ファイル(??)を取りこぼし、コミット済みツリーだけではビルド不能になる(safeStorageLocal.js
  コミット漏れ→Vercel 全デプロイ失敗の実事故)。ローカル verify は作業ツリー基準なので検出できない。
  v0.1.1081 の `check:tracked-imports`(verify:cc 内)が機械検出するが、頼り切らず add 時点で防ぐ。
...
### 12.10 wayfinder→to-spec方式(既存コードの真因調査が要る修正の標準手順・2026-07-30採用)

> 背景: Matt Pocock氏の`/wayfinder`→`/to-spec`(2026-07-29公開)。既存機能のバグ(ちらつき・
> 不整合等)は雑に投げると真因を外した修正になりやすい。まず司令塔が実コードを読んで地図
> (`<お題>-MAP.md`)を書き、その地図をFableに渡して仕様(`<お題>-SPEC.md`)を設計させ、
> 実装ハンドオフを経て実装する2.5段構え。会場応援者ランキングのチラつき修正
> (`venue-ranking-churn-*.md`・v0.1.1190)で初採用、reality-checkerの推移性全数検証・
> 後方互換恒等性のランダム試行検証まで含めて有効性を確認済み。

- **council-fable(会議→Fable)との使い分け**: ゼロから理想像を発散させたい設計は
  council-fable。**既存コードのどこかに構造的な真因があるバグ修正・機能追加**は
  wayfinder→to-spec。迷ったら「このバグは実コードの構造に根っこがあるか」で判断する。
- **正本・使い方**: `github/WAYFINDER-TO-SPEC-HOWTO.md`(横断正本)。グローバルスキル
  `/wayfinder-to-spec 〈お題〉`で起動する。このリポ固有のコピーは作らない。
- **地図の質は司令塔の実コード裏取りに比例する**: サブエージェント(Explore等)に地図を
  書かせてよいが、ファイルパス・行番号・既存関数の挙動は司令塔が実際にコードを読んで
  裏取りしてから採用する。裏取りを省くと幻覚が地図→仕様→実装まで伝播する。
  実例: Fableが「count減少経路の実在は未確認」とした論点を、司令塔が`pruneRoster`
