...
- **直前提出バージョン**: **0.1.75（2026-05-01 提出済 / 審査待ち）**
- **直近通過バージョン**: 0.1.7（2026-04-23 提出 / 審査通過済・公開中）
- **前回提出**: 0.1.10（2026-04-29 提出 / 審査結果が 0.1.75 提出に巻き取られる形に）
- **ステータス**: 0.1.7 が公開中。**2026-05-01 に 0.1.75 を CWS に再提出**（ダッシュボード
 説明文の OpenRouter 表現を「未実装」に修正 + データ使用「個人を特定できる情報」を追加チェック
 で privacy.html §2/§6 と整合）。提出時 manifest 詳細・ダッシュボード入力文字列の正本は
 [docs/releases/cws-submission-texts.md](docs/releases/cws-submission-texts.md) を参照。
 自動公開設定 ON。
- **0.1.10〜0.1.74 の経緯（履歴）**: 0.1.11 は 0.1.10 提出後に
 ...
 サムネ」（F/G）を追加した。**0.1.14〜0.1.74 では HTML レポート / マーケ分析の機能拡充
 （0.1.21〜0.1.25 で分析 28 件投入）、配信者タイル復活、popup window UX 改善、ランキング
 導線等を投入し、最終的に 0.1.75 として 2026-05-01 にまとめて再提出した**。
...
## 5. 直近セッションで入った変更（2026-05-01）

**CWS 0.1.75 提出（コード変更なし・ダッシュボード文章更新）**:

- 2026-05-01 に 0.1.75 を CWS Developer Dashboard へ提出（自動公開設定 ON、審査待ち）。
- 提出前にダッシュボード上の「単一用途の説明」最終文を「AI 連携機能は本バージョン時点で
 未実装です（将来予定）」に書き換え。`tsuioku-no-kirameki/privacy.html` §6「AI 連携機能
 （将来予定・未実装）」と整合（書面齟齬での審査リジェクトを回避）。
- 「データ使用」の **「個人を特定できる情報」にチェックを追加**。privacy.html §2 で
 ニコ生数値ユーザー ID（個人識別番号）の保存を明記している以上、ここを未チェックだと
 文書間齟齬扱いされる可能性があったため。
- ダッシュボードに入力した全文字列（単一用途・各権限の理由・データ使用の宣言）を
 [docs/releases/cws-submission-texts.md](docs/releases/cws-submission-texts.md) に
 「正本」として保存。次回提出時はここから貼り直す運用にする（再構築コスト削減）。
