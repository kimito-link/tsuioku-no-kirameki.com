...
- `docs/repo-tree-map.md`（テキスト正本）＋ `docs/repo-tree-map.html`（色付き視覚ビュー）に2つ載る:
  1. **ディレクトリマップ**（場所 → 役割）: 各ディレクトリが何の担当か（色・速度・コメント・レポート 等）。
  2. **機能逆引き索引**（機能 → 担当ファイル）: 「送信を司るのはここ」「popup スクロールはここ」式に、
     挙動から担当ファイルへ一発で辿れる。
